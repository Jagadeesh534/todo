...
function ListTasks() {
  ...
  const saveEditedData = () => {
    ...
    axios
      .post(BASE_URL + "save-task", taskObj)
      .then(async (response) => {
        if (response.data.message === "success") {
          dispatch(setTodos([]));
          dispatch(fetchTodos());
  ...
  return (
    <div className="d-flex justify-content-center">
      <div className="container w-100 d-flexx">
        {todos?.length === 0 && (
          <h2 className="text-center text-dark ">No Tasks Available</h2>
        )}
        {todos?.map((todo) => (
          <div
            key={todo.id}
            className={`m-1 p-2 text-center d-flex justify-content-between align-items-center border ${
              todo.done ? "border-success" : "border-info"
            } }`}
          >
